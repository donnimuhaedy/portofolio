...
const ProjectSingle = ({ posts }) => {
  ...
  return (
    ...
    >
      {post && (
        <motion.article variants={postVariants}>
          <div className="ProjectSingle-body grid">
            ...
            <aside className="ProjectSingle-sideList sm">
              {post.techStack && (
                <SideList title="TECH STACK" items={post.techStack} />
              )}
            </aside>
          </div>
          <div className="ProjectSingle-hero grid">
            {post.thumbnail && (
              <img src={post.thumbnail.url} alt={post.title} className="py-3" />
            )}
          </div>
          <div className="ProjectSingle-action grid">
            {post.previewUrl && (
              ...
              >
                LIVE PREVIEW <i className="fa fa-external-link"></i>
              </a>
            )}
            {post.sourceUrl && (
              <a
              ...
              >
                SOURCE CODE <i className="fa fa-external-link"></i>
              </a>
            )}
          </div>
        </motion.article>
      )}
      <div className="BackButton btn" style={{ marginTop: "3rem" }}>
        <BackButton route="/projects" title="BACK TO PROJECTS" />
      </div>
    </motion.section>
  );
};
